 ...
 * 
 * Has a number of implementation to creating the video message. This is due to
 * a number of ways that were defined in the RTMP spec.
 * 
 * 
 */
public class Serializer {
	...
	public static byte[] createAMFVideoData(byte[] payload, int timestamp) {
		...
		byte[] payloadLength = Utils.intToByteArray(payload.length); // TODO if
																		// payload
																		// size
																		// is
																		// bigger
	...
	public static void main(String args[]) throws IOException {
		...
		for (int i = 0; i < 1; i++) {
			...
			byte[] data = createAMFVideoData(f.getFrame(), (int) f
					.getTimeStamp());
