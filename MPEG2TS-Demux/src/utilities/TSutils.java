...
public class TSutils {
	...
	public static long getPTS(byte[] tsPacket) {
		...
		pts      = (pts << 15) + ((tsPacket[offset+1] & 0xff) << 7) + ((tsPacket[offset+2] & 0xfe) >> 1);
		pts      = (pts << 15) + ((tsPacket[offset+3] & 0xff) << 7) + ((tsPacket[offset+4] & 0xfe) >> 1);
	...
	public static long getDTS(byte[] tsPacket) {
		...
		dts      = (dts << 15) + ((tsPacket[offset+1] & 0xff) << 7) + ((tsPacket[offset+2] & 0xfe) >> 1);
		dts      = (dts << 15) + ((tsPacket[offset+3] & 0xff) << 7) + ((tsPacket[offset+4] & 0xfe) >> 1);
