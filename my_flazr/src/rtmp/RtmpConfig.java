...
public class RtmpConfig {
	...
	private static void configure(Type type) {
		...
		if (!propsFile.exists()) {
			...
			case SERVER:
			case SERVER_STOP:
				Integer serverStop = parseInt(props
						.getProperty("server.stop.port"));
				if (serverStop != null)
			...
			case PROXY:
			case PROXY_STOP:
				Integer proxyStop = parseInt(props
						.getProperty("proxy.stop.port"));
				if (proxyStop != null)
				...
				PROXY_REMOTE_HOST = props.getProperty("proxy.remote.host",
						"127.0.0.1");
				Integer proxyRemote = parseInt(props
