...
public abstract class RtmpPublisher {
	...
	public RtmpPublisher(final RtmpReader reader, final int streamId,
			final int bufferDuration, boolean useSharedTimer,
		...
		if (useSharedTimer) {
			...
			timer = new HashedWheelTimer(RtmpConfig.TIMER_TICK_SIZE,
	...
	public boolean handle(final MessageEvent me) {
		if (me.getMessage() instanceof Event) {
			...
			if (pe.conversationId != currentConversationId) {
				//logger.debug(
					//	"stopping obsolete conversation id: {}, current: {}",
	...
	public void start(final Channel channel, final int seekTime,
			final int playLength, final RtmpMessage... messages) {
		this.playLength = playLength;
		start(channel, seekTime, messages);
	}

	public void start(final Channel channel, final int seekTimeRequested,
			final RtmpMessage... messages) {
		paused = false;
	...
	//	logger
	//			.debug(
	//					"publish start, seek requested: {} actual seek: {}, play length: {}, conversation: {}",
	//					new Object[] { seekTimeRequested, seekTime, playLength,
	//							currentConversationId });
	...
	private void write(final Channel channel) {
		...
		if (aggregateModeEnabled && clientBuffer > timerTickSize) { // TODO
																	// cleanup
		...
		final double compensationFactor = clientBuffer
				/ (bufferDuration + timerTickSize);
		final long delay = (long) ((header.getTime() - timePosition) * compensationFactor);
		//if (logger.isDebugEnabled()) {
		//	logger
		//			.debug(
		//					"elapsed: {}, streamed: {}, buffer: {}, factor: {}, delay: {}",
		//					new Object[] { elapsedTimePlusSeek, timePosition,
		//							clientBuffer, compensationFactor, delay });
		//}
		...
		future.addListener(new ChannelFutureListener() {
			...
			public void operationComplete(final ChannelFuture cf) {
				...
				if (completedIn > 2000) {
					//logger
					//		.warn(
					//				"channel busy? time taken to write last message: {}",
					//				completedIn);
				}
				final long delayToUse = clientBuffer > 0 ? delay - completedIn
						: 0;
	...
	public void fireNext(final Channel channel, final long delay) {
		...
		if (delay > timerTickSize) {
			timer.newTimeout(new TimerTask() {
				...
				//		logger
				//				.debug("pending 'next' event found obsolete, aborting");
						return;
					}
	...
	private void stop(final Channel channel) {
		...
		//		new Object[] { seekTime / 1000, elapsedTime / 1000,
		//				(timePosition - seekTime) / 1000 });
